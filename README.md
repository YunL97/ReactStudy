...
* 리액트에서는 컴포넌트라는 개념을 활용해 독립적인 코드블록을 만들고 나서 그 블록을 조합할 수 있다.
* 리액트에서는 커스텀 컴포넌트가 대문자로 시작해야한다 -> 리액트는 div나 h1과 같은 내장 html 요소와 커스텀 요소를 대문자로구분하기 때문
*  html 태그안에 {} 사용하면 자바스크립트로 실행
*  spa가 발전하면서 검색엔진이 spa 콘텐츠를 인덱싱 할 수 있도록 발전했지만 몇가지 문제점이 있아
   *  초기로딩속도: spa는 초기에 모든 필요한 자원을 한번에 다운로드 하지않고 페이지를 동적으로 로드함 -> 촉디 로딩속도 느림 -> 검색엔진은 로딩속도가 빠른것을 선호함 -> 검색엔진이 페이지를 인덱싱하는데 어려움이 있음
   *  spa 는 동적으로 컨텐츠를 생성하기 때문에 html에 필요한 메타데이터가 부족할 수 있음
...
* react를 데이터베이스에 직접 연결하지 않는거는 보안때문 -> 애플리케이션에 작성하는 모든 코드는 결국 페이지 방문자에게 노출되기 때문
* some 내장함수: js에 포함된 내장 메서드 함수를 인수로 받아 배열 내의 각 항목마다 함수를 실행, 그리고 true, or false 반환
* 
